 ...
 * @brief 測試所有日誌級別的輸出
 */
static void testLogLevels(void) {
    DMS_LOG_INFO("=== Log Level Verification Test ===");
    
    /* 顯示當前級別 */
    DmsLogLevel_t currentLevel = dms_log_get_level();
    DMS_LOG_INFO("Current log level: %s", dms_log_level_string(currentLevel));
    
    DMS_LOG_INFO("Testing all log types at current level:");
    
    /* 測試所有級別的日誌 */
    DMS_LOG_ERROR("❌ This is an ERROR message");
    DMS_LOG_WARN("⚠️  This is a WARN message");
    DMS_LOG_INFO("ℹ️  This is an INFO message");
    DMS_LOG_DEBUG("🔍 This is a DEBUG message");
    
    /* 測試模組化日誌 */
    DMS_LOG_SHADOW("📋 This is a SHADOW module message");
    DMS_LOG_API("🌐 This is an API module message");
    DMS_LOG_MQTT("📡 This is an MQTT module message");
    DMS_LOG_TLS("🔐 This is a TLS module message");
    DMS_LOG_CRYPTO("🔑 This is a CRYPTO module message");
    
    DMS_LOG_INFO("=== Log Level Test Completed ===");
}

/**
 * @brief 測試不同級別設定的效果
 */
static void testDynamicLogControl(void) {
    DMS_LOG_INFO("=== Dynamic Log Level Control Test ===");
    
    /* 保存原始級別 */
    DmsLogLevel_t originalLevel = dms_log_get_level();
    
    /* 測試各個級別 */
    const char* levels[] = {"ERROR", "WARN", "INFO", "DEBUG"};
    DmsLogLevel_t levelValues[] = {
        DMS_LOG_LEVEL_ERROR, 
        DMS_LOG_LEVEL_WARN, 
        DMS_LOG_LEVEL_INFO, 
        DMS_LOG_LEVEL_DEBUG
    ...
    for (int i = 0; i < 4; i++) {
        printf("\n--- Testing log level: %s ---\n", levels[i]);
        dms_log_set_level(levelValues[i]);
        
        /* 送出測試訊息 */
        DMS_LOG_ERROR("ERROR: Always visible");
        DMS_LOG_WARN("WARN: Visible at WARN and above");
        DMS_LOG_INFO("INFO: Visible at INFO and above");
        DMS_LOG_DEBUG("DEBUG: Only visible at DEBUG level");
        
        printf("--- End of %s level test ---\n", levels[i]);
    ...
    /* 恢復原始級別 */
    dms_log_set_level(originalLevel);
    DMS_LOG_INFO("Log level restored to: %s", dms_log_level_string(originalLevel));
    DMS_LOG_INFO("=== Dynamic Log Control Test Completed ===");
}

/**
 * @brief 執行完整的日誌系統測試
 */
static void runLogSystemTests(void) {
    DMS_LOG_INFO("🧪 === Starting Log System Tests ===");
    ...
    /* 測試 2：動態級別控制 */
    testDynamicLogControl();
    
    DMS_LOG_INFO("🏁 === All Log System Tests Completed ===");
}
...
/**
 * @brief 從 MAC 地址計算數字 seed
 * @param macAddress MAC 地址字串 (格式: "AB:A1:AE:69:2A:AE")
 * @return 計算出的數字 seed
 */
uint32_t calculateSeedFromMac(const char* macAddress) {
    ...
    if (!macAddress) {
        DMS_LOG_WARN("MAC address is NULL, using default seed");
        return 12345; /* 預設 seed */
    }
    
    /* 從 MAC 地址的每個字元計算 seed */
    for (int i = 0; macAddress[i] != '\0'; i++) {
        seed += (uint32_t)macAddress[i];
        seed = (seed * 31) % 1000000; /* 避免溢位，保持在合理範圍 */
    }
    
    DMS_LOG_DEBUG("MAC seed calculation: %s -> %u", macAddress, seed);
    return seed;
}

/**
 * @brief 初始化 MAC 地址 seed
 * @param reconnectState 重連狀態結構指標
 */
void initializeMacAddressSeed(ReconnectState_t* reconnectState) {
    if (!reconnectState) {
        DMS_LOG_ERROR("reconnectState is NULL");
        return;
    }
    ...
    /* 嘗試取得真實 MAC 地址 */
    if (getRealMacAddress(macAddress, sizeof(macAddress)) == 0) {
        SAFE_STRNCPY(reconnectState->macAddressSeed, macAddress, sizeof(reconnectState->macAddressSeed));
        DMS_LOG_INFO("Real MAC address obtained: %s", macAddress);
    } else {
        /* 從 CLIENT_IDENTIFIER 中提取 MAC 地址 */
        const char* clientId = CLIENT_IDENTIFIER;
        const char* macStart = strstr(clientId, "benq-dms-test-");
        if (macStart) {
            macStart += strlen("benq-dms-test-");
            /* 轉換 12 位格式到標準格式 */
            if (strlen(macStart) >= 12) {
                snprintf(reconnectState->macAddressSeed, sizeof(reconnectState->macAddressSeed),
                        "%c%c:%c%c:%c%c:%c%c:%c%c:%c%c",
                        macStart[0], macStart[1], macStart[2], macStart[3],
                        macStart[4], macStart[5], macStart[6], macStart[7],
                        macStart[8], macStart[9], macStart[10], macStart[11]);
                DMS_LOG_INFO("MAC address extracted from CLIENT_ID: %s", reconnectState->macAddressSeed);
            } else {
                /* 使用預設 MAC 地址 */
                SAFE_STRNCPY(reconnectState->macAddressSeed, "AB:A1:AE:69:2A:AE", sizeof(reconnectState->macAddressSeed));
                DMS_LOG_WARN("Using default MAC address: %s", reconnectState->macAddressSeed);
            }
        } else {
            /* 使用預設 MAC 地址 */
            SAFE_STRNCPY(reconnectState->macAddressSeed, "AB:A1:AE:69:2A:AE", sizeof(reconnectState->macAddressSeed));
            DMS_LOG_WARN("Cannot extract MAC from CLIENT_ID, using default: %s", reconnectState->macAddressSeed);
        }
    }
    
    /* 計算數字 seed */
    reconnectState->seedValue = calculateSeedFromMac(reconnectState->macAddressSeed);
    DMS_LOG_INFO("MAC seed initialized: %s (seed: %u)", reconnectState->macAddressSeed, reconnectState->seedValue);
}

/**
 * @brief 基於 MAC 地址計算退避延遲
 * @param retryCount 重試次數
 * @param macSeed MAC 地址字串
 * @return 退避延遲時間（秒）
 */
uint32_t calculateBackoffDelayWithSeed(uint32_t retryCount, const char* macSeed) {
    /* 計算基礎延遲 */
    uint32_t baseDelay = RETRY_BACKOFF_BASE_SECONDS * (1 << retryCount);
    
    /* 從 MAC 地址計算隨機偏移 */
    uint32_t seed = calculateSeedFromMac(macSeed);
    uint32_t randomOffset = (seed % MAC_SEED_MAX_OFFSET) * MAC_SEED_MULTIPLIER;
    
    /* 計算最終延遲時間 */
    uint32_t finalDelay = MIN(baseDelay + randomOffset, RETRY_BACKOFF_MAX_SECONDS);
    
    DMS_LOG_DEBUG("Backoff calculation: retry=%u, base=%u, offset=%u, final=%u", 
                  retryCount, baseDelay, randomOffset, finalDelay);
...
/**
 * @brief 更新重連延遲時間（使用新的 MAC seed 策略）
 * 這個函數用來替換或更新現有的重連邏輯
 */
static void updateReconnectDelay(ReconnectState_t* reconnectState) {
    if (!reconnectState) {
        return;
    }
    
    /* 使用新的基於 MAC seed 的退避計算 */
    reconnectState->nextRetryDelaySeconds = 
        calculateBackoffDelayWithSeed(reconnectState->retryCount, reconnectState->macAddressSeed);
    
    DMS_LOG_INFO("Next reconnection in %u seconds (retry #%u, MAC seed: %s)", 
                 reconnectState->nextRetryDelaySeconds, 
                 reconnectState->retryCount,
                 reconnectState->macAddressSeed);
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            printf("  --test             Run connection test\n");
            printf("  --register         Manual device registration\n");
            printf("  --status           Show device status\n");
            printf("  --config           Show configuration\n");
            printf("  --pincode          Show pairing PIN code\n");
            goto cleanup;
        } else if (strcmp(argv[i], "--config") == 0) {
            printf("📋 === Configuration Display ===\n");
            /* 🔧 修正：直接顯示配置，不調用 showConfiguration() */
            const dms_config_t* cfg = dms_config_get();
            if (cfg) {
                printf("AWS IoT Configuration:\n");
                printf("  Endpoint: %s\n", cfg->aws_iot.aws_endpoint);
                printf("  Client ID: %s\n", cfg->aws_iot.client_id);
                printf("  MQTT Port: %u\n", cfg->aws_iot.mqtt_port);
                printf("  Keep Alive: %u seconds\n", cfg->aws_iot.keep_alive_seconds);
                printf("\nDMS API Configuration:\n");
                printf("  Base URL: %s\n", cfg->api.base_url);
                printf("  Product Type: %s\n", cfg->api.product_type);
                printf("  Timeout: %u ms\n", cfg->api.timeout_ms);
                printf("\nReconnection Configuration:\n");
                printf("  Max Attempts: %u\n", cfg->reconnect.max_retry_attempts);
                printf("  Base Delay: %u seconds\n", cfg->reconnect.base_delay_seconds);
                printf("  Max Delay: %u seconds\n", cfg->reconnect.max_delay_seconds);
            } else {
                printf("❌ Configuration not available\n");
            }
            goto cleanup;
        } else if (strcmp(argv[i], "--test") == 0) {
            printf("🧪 === Connection Test Mode ===\n");
            returnStatus = runConnectionTest();
            goto cleanup;
        } else if (strcmp(argv[i], "--register") == 0) {
            printf("📝 === Manual Registration Mode ===\n");
            returnStatus = runManualRegistration();
            goto cleanup;
        } else if (strcmp(argv[i], "--status") == 0) {
            printf("📊 === Device Status Mode ===\n");
            returnStatus = showDeviceStatus();
            goto cleanup;
        } else if (strcmp(argv[i], "--pincode") == 0) {
            printf("🔢 === PIN Code Mode ===\n");
            /* 簡化處理，避免調用可能不存在的函數 */
            printf("PIN Code functionality not implemented yet\n");
            goto cleanup;
        } else {
    ...
    g_reconnectState.retryCount = 0;
    g_reconnectState.nextRetryDelaySeconds = RETRY_BACKOFF_BASE_SECONDS;
    g_reconnectState.totalReconnects = 0;
    ...
    if (dms_shadow_wait_get_response(SHADOW_GET_TIMEOUT_MS) == DMS_SUCCESS) {
        ...
        if (dms_shadow_is_device_bound()) {
            ...
            printf("   → DMS registration required\n");
            
            /* 自動註冊流程 */
            printf("\n=== Step 2.1: Auto-Registration ===\n");
            if (checkAndRegisterDevice() == DMS_SUCCESS) {
                printf("✅ Auto-registration completed successfully\n");
            } else {
                printf("⚠️  Auto-registration failed, device will operate with limited functionality\n");
                printf("   Use 'dms-client --register' for manual registration\n");
            }
        }
    } else {
        printf("⚠️  Shadow Get response timeout (this is not critical for operation)\n");
    }

#ifdef BCML_MIDDLEWARE_ENABLED
    DMS_LOG_INFO("🔧 Initializing BCML Middleware integration...");
    if (bcml_adapter_init() != DMS_SUCCESS) {
        printf("❌ Failed to initialize BCML adapter\n");
        /* 不直接退出，而是切換到模擬模式 */
        printf("⚠️  Continuing with simulation mode\n");
    } else {
        DMS_LOG_INFO("✅ [BCML] Middleware integration ready");
    }
#else
    printf("🎭 Running in simulation mode (BCML disabled)\n");
#endif
...
cleanup:
    ...
    printf("\n🏁 === DMS Client Exiting (New Module Integration) ===\n");
    printf("   Status: %d\n", returnStatus);
    printf("   Total Reconnects: %u\n", g_reconnectState.totalReconnects);
